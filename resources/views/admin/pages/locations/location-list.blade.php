    ...
    <div class="ps-3">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 p-0 align-items-center">
          ...
          <li class="breadcrumb-item active" aria-current="page">Location Record</li>
        </ol>
      </nav>
    </div>
    <div class="ms-auto">
                    ...
                    <table id="example2" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            @foreach($locations as $row)
                                <tr>
                                    <td>{{$row->project}}</td>
                                    <td> <img src="{{ asset('public/assets/images/projects/location') }}/{{ $row->icon }}" alt="" style="width:50px"></td>
                                    <td>{{$row->name}}</td>
                                    <td>
                                        <a href="{{route('locations.edit',$row->id)}}" class="text-warning btn btn-default" data-bs-toggle="tooltip" data-bs-placement="bottom" title="" data-bs-original-title="Edit info" aria-label="Edit">
                                            <ion-icon name="pencil-sharp" role="img" class="md hydrated" aria-label="pencil sharp"></ion-icon>
                                          </a>
                                    {!! Form::open(['method' => 'DELETE','route' => ['locations.destroy', $row->id],'style'=>'display:inline']) !!}
